...
<div class="PostForm">
    <div class="ContentForm">
        <div class="TopContent">
            <input class="Title" type="text" name="title" id="title" placeholder=" 제목을 입력해 주세요."
                   value="<c:out value='${recipe.title}'/>" readonly/>
            ...
            <div class="LikeModify">
                ...
                <c:if test="${sessionScope.id eq recipe.writer}">
                    ...
                    <button type="submit" class="btn btn-warning" id="ModifyRegisterBtn" onclick="modifyButtons()"
                            style="display: none;">등록
                    </button>
                </c:if>
            </div>
        </div>
        <textarea class="form-control Content" name="content" id="content" rows="10" cols="100" readonly><c:out
                value='${recipe.content}'/></textarea>
    </div>
    ...
    <div class="FtnBtn">
        ...
        <a href="<c:url value='/recipeboard${pi.getPageQuery(pi.page,pi.pageBundle)}'/>">
            목록
        </a>
    </div>
</div>
